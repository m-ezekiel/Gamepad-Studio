...
# Import data
txt_files <- list.files("Data/gameplay_data_V/", pattern = "txt")
png_files <- list.files("Data/gameplay_data_V/", pattern = "png")

file_path <- paste0("Data/gameplay_data_V/", txt_files[1])
...
for (i in 1:length(txt_files)) {
  file_path <- paste0("Data/gameplay_data_V/", txt_files[i])
  gpg_data <- read.table(file_path, sep = "\t", header = TRUE)
...
# Write data
write.table(x = player_data, file = "player_data_V.tsv", sep = "\t")
